...
import discord  # discord 모듈을 불러옵니다. (discord.py 기능을 사용하기 위함입니다.)

client = discord.Client()   # 디스코드 클라이언트와의 연결입니다. 해당 연결을 client 변수에 저장합니다.
token = "봇 토큰"  # 봇 토큰은 양 옆 따옴표('')나 큰 따옴표("")로 감싸져, 문자열(string)형태여야 합니다.

@client.event   # @(골뱅이)는 데코레이터(decorator)라고 부릅니다. 위의 설명문을 참고하세요.
async def on_message(message):  # on_message, 봇이 참여한 서버에 메시지가 보내졌을 때마다 작동합니다.
    if message.author.bot:  # 메시지를 보낸 사용자가 봇이라면 :
        return              # 아무 행동도 하지 않는다.

    if message.content == "!ping":  # 비교 연산자, [ == ] 입니다. 뜻은 "값이 동일하다", "값이 정확히 일치하다" 입니다.
        await message.channel.send("pong!") # 사용자가 보낸 메시지의 내용이 정확히 [ !ping ] 이라면, 해당 메시지 채널에 [ pong! ] 이라는 메시지를 보냅니다.

    if message.content.startswith("!안녕"):   # startswith() 함수이며, 사용자가 보낸 메시지의 내용이 [ !안녕 ] 으로 시작한다면 아래 구문을 실행합니다.
        await message.channel.send("안녕하세요.")

    if message.content.endswith("안녕!"):     # endswith() 함수이며, 사용자가 보낸 메시지의 내용이 [ 안녕! ] 으로 끝난다면 아래 구문을 실행합니다.
